...
# Hosts a local server on port 8000, written in Go 1.23, and packages into a container image.
...
# Builds the binaries
RUN  CGO_ENABLED=0 GOOS=linux go build -o /go-fetch-backend .
...
COPY --from=base /go-fetch-backend .
